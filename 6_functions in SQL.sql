...
SELECT coalesce(state,'N/A') from customers;

SELECT customerName, city, COALESCE(state, 'Not available') as State, country
FROM  customers;
...
SELECT CURDATE();
SELECT CURRENT_DATE(),CURRENT_DATE, CURDATE(); -- all give same output
...
SELECT COALESCE(NULL, NULL, NULL, 'W3Schools.com', NULL, 'Example.com') as COALESE_func;
